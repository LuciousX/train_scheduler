...
var index = 0;
  ...
  var config = {
    apiKey: "AIzaSyAVeB0qjj39tZLcwHF_QZb4KxHzMQPYAhA",
    authDomain: "train-schedule-fb5df.firebaseapp.com",
    databaseURL: "https://train-schedule-fb5df.firebaseio.com",
    projectId: "train-schedule-fb5df",
    storageBucket: "train-schedule-fb5df.appspot.com",
    messagingSenderId: "487599711279"
  };
...
database.ref().orderByChild("dateAdded").on("child_added", function (snapshot) {
  ...
  var updateButton = $("<button>").html("<span class='glyphicon glyphicon-edit'></span>").addClass("updateButton").attr("data-index", index).attr("data-key", snapshot.key);
  var removeButton = $("<button>").html("<span class='glyphicon glyphicon-remove'></span>").addClass("removeButton").attr("data-index", index).attr("data-key", snapshot.key);

  var firstTime = snapshot.val().firstTime;
  var trnFrequency = parseInt(snapshot.val().frequency);
  ...
  var minutesRemaining = trnFrequency - tRemainder;
  ...
  if ((currentTimeCalc - firstTrain) < 0) {
    ...
    minutesRemaining = trnFrequency - tRemainder;
    console.log("Working");
  }


  var tabRow = $("<tr>");
  tabRow.addClass("row-" + index);
  var cell1 = $("<td>").append(updateButton);
  var cell2 = $("<td>").text(snapshot.val().name);
  var cell3 = $("<td>").text(snapshot.val().destination);
  var cell4 = $("<td>").text(snapshot.val().frequency);
